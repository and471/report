...
\title{Visualisation Catalogue \\ \vspace{10pt}
\textit{\large Integrated platform for visualistion submission, scheduling and visualisation playout} \\ \vspace*{-5pt}
...
We are acutely aware that with Computing examinations being held at the
...
the project. As a group, we allowed some leeway for each task in case we run into technical errors and difficulties. If this was the case, we communicated and came up with a solution if necessary.
...
\begin{enumerate}
  ...
  \item \textbf{To strictly adhere to the original project scope}: While we believe that the
...
Taking into account the current project scope, the resultant plan and timeline shown in figure 
...
Estimates on time required for the tasks are based on their relative size and
time taken for us to complete similar ones in the past. For example, some of us
have implemented a College (Kerberos) Login System well within a week, so we inferred that the team (now double the size) is capable of 
fitting two size M tasks within an iteration. For larger system modules, we
assigned a longer period specifically for that task: we expected that the scheduling
system (size L) and the entire playout system (size XL) will take us one and
...
a part of the project they perferred to work on. We found that there was a
good split of two people (Andrew \& Emma) that wanted to work on the frontend,
two (Thomas \& Timothy) on the backend server code, and two (Bryan \& Jia Guang) focusing on the database and scheduling.

Although this is a decent distribution to initiate work, we realised that the frontend 
aspect of the project may require more work in later iterations.
In addition, we expected the server code and database to be fully 
implemented within the first four iterations, requiring only minor fixes thereafter.
...
Towards the end of the project, we used the issue tracker on Gitlab to inform the group about small bugs.
Although these could be assigned to a specific member, we left them open to the group, so that an
...
would like to use, as we felt that this project should also be an opportunity to explore new
...
The member will also be responsible for implementing the features with the technology should it be
deemed fit.



\subsection{Development Methods} \label{sec:projman_devprocess}
We have agreed to adopt a mixture of agile development methodologies to
ensure our practical needs and assist the group's working flow.

\subsubsection{General Project Management}

We generally followed the Scrum method for project management and adopted the
following characteristics:
\begin{itemize}
  \item \textbf{Roles}: We see Dr. David Birch, our supervisor, as the \textit{Product Owner}
        who provides ideas and feedback for the development. At the same time,
        we agreed that Andrew and Bryan should assume the role of
        \textit{Scrum Masters} to facilitate work.
  \item \textbf{Ceremonies}: After the initial meeting, we met David each Thursday to review our previous sprint 
        and plan our next sprint. Also, we held a development team
...
We also integrated a few technical practices inspired by Extreme Programming:
\begin{itemize}
  \item \textbf{Pair Programming}: As mentioned in section \ref{sec:projman_group}, we arranged
        ourselves into three pairs, focusing on frontend (FE), backend (BE) and database (DB) work.
        Such a practice allowed continuous development on all fronts without
        being affected by instances in which a member is required to temporarily shift his or her
        focus from the project to other coursework/tests. All members are open to team up
        with different members for different tasks, though it turned out that group members with similar
        preferences usually end up in the same pairs for different tasks.
  \item \textbf{Test-driven Development}: During development, we always wrote unit and system tests prior to conducting the actual implementation. This ensuresd that individual features are working as expected before we moved on to the next feature.
...
Finally, to keep track of the group's progress, we used an electronic task
board on Trello (see figure \ref{fig:eval_trello}), which enabled us to assign ourselves (and others) specific tasks and a general due date. Trello allowed us to see what the other members of the group are doing and when a feature is expected to be completed. We can also easily add photos and checklists to see how the project is forming. We decided to use Trello as a physical story-board is not feasible. As well as Trello, we constantly communicated personally in labs, online via messaging services (mainly Facebook Messenger), as well as regular stand-ups in a meeting room.
...
In a group project last year, the backend team had to rely on a functional frontend to confirm that a 
particular feature was working. Utilising the newly available CloudStack from the department,
we were able to create a number of virtual machines (VMs) for each feature while it is in development,
and have multiple copies of the web server running from each VM respectively.

While the VMs come nearly for free, they might come with slightly different specifications,
which might cause discrepancies further into development. After a lecture on deployment techniques, we decided to use Docker, as it allows us to quickly set up standardised VMs (containers), installing any dependencies we need automatically. Although it was a steep learning curve at the start of our project, using Docker saved us a lot of time in many ways, such as ensuring that all supporting libraries exist in the local VMs that we were using.

To further speed up the development process, we also wrote a number of automated scripts
to build Docker images (templates for a container VM), as well as scripts to start and stop the
web server when called.

\subsubsection{Version Control}
We have decided to continue using Git as our version control system, as all of us have had
...
allowing one to continue their work should a linux machine (for example) be unavailable.

As usual, all of our code (including the scripts mentioned in \ref{sec:projman_devenv}) 
is stored in a group git repository on the department's GitLab server. Instead of naming branches after each team member's
name and working in it individually for the entire project, we have decided to use branches with a short lifetime.
Each minor feature is implemented in its own branch, and the master branch is only used for merging features for release
...
It also makes it easier for the team to backtrack and locate bugs should any problems arise, by checking a small number of commits on that particular branch.

Moreover, with meaningful yet concise commit messages, wiki residing in repository and issues, we can effectively
communicate with each other on what we have done, along with updating our task-board
...
RSpec is inevitably the best choice for testing server codes, as it is nicely integrated with Ruby on Rails, with test templates automatically
generated along with new code segments (models and controllers).
Furthermore, tests written with RSpec are highly human-readable, thus
easing the difficulty in maintainence as the tests themselves provide a
decent documentation of our implementation.

We have also written an automated script to run all tests in a seperate
...
As it is cumbersome to manually stop, test and restart the server
every time we implement a feature or provide a fix, we have
...
Jenkins will notify us immediately, so that we can get
...
\subsection{Visualisation Catalogue Implemetation}
...
\begin{figure}[h!]
  ...
  \caption{System architecture for Visualisation Catalogue}
...
\begin{enumerate}
  ...
  \item The original set up of the project was hampered by the use of Python and Flask. As described 
  before, the switching of technologies set us back one week of work at the very beginning of the
  project.
...
We are planning to extend this user testing to include more potential users, especially those that are ranked second in our list of stakeholders, such as staff and students who may use the platform to submit and view visualisations.

As the William Penney building is not completed yet, it is hard to perform user testing with those
walking on the Sherfield Walkway. To test the playout system, we as a group simulated the projectors on 
a browser and checked if the displayed visualisations were in line with the output from the scheduling 
algorithm.

%TODO: what did these tests show? did it work?
...
%TODO: what would we have done differently, what did we learn? more!
Reflecting on our project as a whole, we are happy with our final project. As the switching from Flask 
to Rails cost us a week of work, we concluded that more research was needed into Flask before we 
started. Also, when choosing technologies to work with, more weight should be put on past experience. We
found that a good balance between learning new techniques (AngularJS) and building upon old knowledge 
(Ruby on Rails) is key to a successful project. 

We also found that we were slightly behind on developing for the playback system, and should have
started work slightly earlier. Using our Trello board, we should have reassigned tasks to people 
capable of implementing them quicker, and moved other members onto the playback system. 
